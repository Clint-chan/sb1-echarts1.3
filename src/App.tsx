...
import { BarChart3, PieChart, LineChart, Github, ListTodo, KeyRound, Calendar, Binary } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div
        className={`fixed inset-0 bg-black/30 transition-opacity ${
          isTodoOpen || isPasswordOpen || isCrontabOpen || isUnicodeOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
      ...
      <div
        className={`fixed top-0 right-0 h-full w-80 bg-white shadow-lg transform transition-transform duration-300 ease-in-out ${
          isUnicodeOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <UnicodeConverter onClose={() => setIsUnicodeOpen(false)} />
      </div>
    </div>
  );
}